...
export const authService = {
  ...
  async verifyEmail(token: string) {
    try {
      ...
      try {
        ...
        console.log(
          "Verification success via GET with token in URL:",
          response.data
        );
        ...
        try {
          ...
          console.log(
            `Attempting POST ${API_URL}/auth/verify-email with token in body`
          );
          response = await axios.post(`${API_URL}/auth/verify-email`, {
            token,
          });
          console.log(
            "Verification success via POST with token in body:",
            response.data
          );
          return { success: true, data: response.data };
        } catch (secondAttemptError) {
          console.log(
            "Second verification attempt failed, trying GET with query param"
          );

          // Try token as query parameter
          console.log(
            `Attempting GET ${API_URL}/auth/verify-email?token=${token}`
          );
          response = await axios.get(
            `${API_URL}/auth/verify-email?token=${token}`
          );
          console.log(
            "Verification success via GET with token as query param:",
            response.data
          );
          return { success: true, data: response.data };
        }
  ...
  async handleVerificationRedirect(token: string, callbackUrl?: string) {
    // Get the base URL
  ...
  // Verify vendor invitation
  async verifyVendorInvitation(token: string) {
    try {
      const response = await axios.get(`${API_URL}/vendors/verify-invitation`, {
        params: { token },
      });
      return response.data;
    } catch (error: any) {
      throw new Error(
        error.response?.data?.message || "Failed to verify vendor invitation"
      );
    }
  },
  ...
  async setupVendor(
    ...
    try {
      const response = await axios.post(`${API_URL}/vendors/onboarding`, {
        token,
        ...data,
      });
      return response.data;
      ...
      throw new Error(
        error.response?.data?.message || "Failed to complete vendor setup"
      );
    }
  },
};
