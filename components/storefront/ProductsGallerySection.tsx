        ...
        return (
          <div key={cat._id}>
            <div className="flex items-center">
              <button
                className={`flex-1 text-left font-medium transition py-2 ${isSelected ? 'text-brand-yellow font-semibold' : 'text-gray-700 hover:text-brand-yellow'}`}
...
export default function ProductsGallerySection({ initialProducts }: ProductsGallerySectionProps) {
  ...
  const brandOptions = [{ label: 'All', value: 'All' }, ...brandsResponse.map((b: any) => ({ label: b.name, value: b._id }))];
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory !== 'All') {
      result = result.filter(p =>
        (typeof p.category === 'object' && p.category !== null && '_id' in p.category && p.category._id === selectedCategory) ||
    ...
    if (selectedReview !== 'All') {
      result = [...result].sort((a, b) => {
        if (selectedReview === 'review-desc') return (b.rating || 0) - (a.rating || 0);
        if (selectedReview === 'review-asc') return (a.rating || 0) - (b.rating || 0);
        return 0;
      });
    }
  ...
  const getColorLabel = (val: string) => colorOptions.find(c => c === val) || val;
  ...
  const hasActiveFilters = [selectedBrand, selectedColor, selectedReview, selectedPrice].some(f => f !== 'All');
    ...
    <div className="max-w-6xl mx-auto px-2 flex flex-col ">
            ...
            <div className="flex flex-wrap gap-2 mb-1">
              {selectedBrand !== 'All' && (
                <span className="bg-brand-yellow/90 text-gray-900 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                  ...
                  <button onClick={() => setSelectedBrand('All')} className="ml-1 text-gray-700 hover:text-red-600">&times;</button>
                </span>
              )}
              {selectedColor !== 'All' && (
                <span className="bg-brand-yellow/90 text-gray-900 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                  Color: {getColorLabel(selectedColor)}
                  <button onClick={() => setSelectedColor('All')} className="ml-1 text-gray-700 hover:text-red-600">&times;</button>
                </span>
              )}
              {selectedReview !== 'All' && (
                <span className="bg-brand-yellow/90 text-gray-900 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                  {getReviewLabel(selectedReview)}
                  <button onClick={() => setSelectedReview('All')} className="ml-1 text-gray-700 hover:text-red-600">&times;</button>
                </span>
              )}
              {selectedPrice !== 'All' && (
                <span className="bg-brand-yellow/90 text-gray-900 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                  {getPriceLabel(selectedPrice)}
                  <button onClick={() => setSelectedPrice('All')} className="ml-1 text-gray-700 hover:text-red-600">&times;</button>
                </span>
              )}
              <button onClick={resetFilters} className="ml-2 px-3 py-1 rounded bg-gray-200 text-gray-700 hover:bg-brand-yellow hover:text-white transition text-xs font-medium">Clear All</button>
            </div>
          )}
          <div className="flex flex-wrap gap-2 items-center justify-end w-full">
            {/* Color filter */}
            <select
              className={`w-[120px] border rounded-full bg-[#F3F3F3] px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-brand-yellow ${selectedColor !== 'All' ? 'border-brand-yellow bg-yellow-50 font-semibold' : 'border-gray-300'}`}
              value={selectedColor}
              onChange={e => { setSelectedColor(e.target.value); updateUrlFilter('color', e.target.value); }}
            >
              <option value="All">All Colors</option>
              {colorOptions.filter(opt => opt !== 'All').map(opt => (
                <option key={opt} value={opt}>{opt}</option>
              ))}
            </select>
            {/* Brand filter */}
            <select
              className={`w-[120px] border rounded-full bg-[#F3F3F3] px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-brand-yellow ${selectedBrand !== 'All' ? 'border-brand-yellow bg-yellow-50 font-semibold' : 'border-gray-300'}`}
              value={selectedBrand}
              onChange={e => { setSelectedBrand(e.target.value); updateUrlFilter('brand', e.target.value); }}
            >
              <option value="All">All Brands</option>
              {brandOptions.filter(opt => opt.value !== 'All').map(opt => (
                <option key={opt.value} value={opt.value}>{opt.label}</option>
              ))}
            </select>
            {/* Review sort */}
            <select
              className={`w-[150px] border rounded-full bg-[#F3F3F3] px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-brand-yellow ${selectedReview !== 'All' ? 'border-brand-yellow bg-yellow-50 font-semibold' : 'border-gray-300'}`}
              value={selectedReview}
              onChange={e => { setSelectedReview(e.target.value); updateUrlFilter('review', e.target.value); }}
            >
            ...
            {/* Price sort */}
            <select
              className={`w-[150px] border rounded-full bg-[#F3F3F3] px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-brand-yellow ${selectedPrice !== 'All' ? 'border-brand-yellow bg-yellow-50 font-semibold' : 'border-gray-300'}`}
              value={selectedPrice}
              onChange={e => { setSelectedPrice(e.target.value); updateUrlFilter('price', e.target.value); }}
            >
      ...
      <div>
        ...
        <aside className="sticky top-4 self-start w-full md:w-[300px] bg-white border-r border-gray-200 p-4 overflow-y-auto max-h-[80vh] rounded-xl shadow-sm hidden md:block">
          ...
          <button
            className={`mb-2 px-3 py-1 rounded ${selectedCategory === 'All' ? 'bg-brand-yellow text-white' : 'bg-gray-100 text-gray-700'} w-full text-left`}
          ...
          {categoryOpen && (
            <RecursiveCategoryList
              ...
              openCategoryIds={openCategoryIds}
              toggleCategory={toggleCategory}
              onCategorySelect={cat => { setSelectedCategory(cat._id); updateUrlFilter('category', cat._id); }}
              selectedCategory={selectedCategory}
            />
          )}
          {/* Brand filter */}
          ...
          {isLoading
            ...
            : filteredProducts.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center py-16 text-gray-400">
                  ...
                  <span className="text-lg font-medium">No products found or available</span>
                </div>
              ) : (
